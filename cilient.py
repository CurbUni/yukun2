...
import rospy
#倒入自定义的数据类型
from topic.msg import topic_msg
...
def talker():
    ...
    pub = rospy.Publisher('topic_msg_info', topic_msg , queue_size=10)
    ...
    while not rospy.is_shutdown():
        #计算距离 
        rospy.loginfo('teacher_JCJ: message: x = %f ,y = %f',x,y)  #rospy中的打印函数
        pub.publish(topic_msg(state,x,y))
        x=1.03*x
        y=1.01*y
        rate.sleep()
